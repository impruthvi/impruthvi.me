...
### Laravel May Be Exploring This Too

Interestingly, the same idea is already being explored in the Laravel ecosystem.

[Pushpak](https://pushpak1300.me/) opened a [draft pull request to the Laravel AI SDK](https://github.com/laravel/ai/pull/1010) proposing a new `Classification` API built around the same kinds of decision primitives discussed in this article: `Boolean`, `Choice`, and `Score`.
...
$result = Classification::of($supportRequest)
    ->questions([
        'urgent' => new Boolean(
            'Does this request need an immediate response?'
        ),
